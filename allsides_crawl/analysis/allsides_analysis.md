...
## AP News Scraper Patch Notes (2026-06-15)

**Patched file:** `per_domain/apnews.com.patched.json` (original `apnews.com.json` untouched)

### Changes applied
1. **Headings included** — `<h2>` section headings are now extracted into `extracted_body_text` as `## Heading` markers.
2. **Text spacing fixed** — `get_text(" ", strip=True)` prevents merged words (e.g. "advancePresident" → "advance President").
3. **Images extracted** — `extracted_images` field added with lead and inline article images. Uses highest-resolution non-webp `<source>` srcset URL. Placeholder `data:image/svg+xml` URLs are filtered out.
4. **Interactive embeds extracted** — `extracted_interactives` field captures AP interactive embed URLs (Datawrapper charts, maps) from `<iframe>` elements in `HTMLModuleEnhancement` containers. Duplicates are deduplicated.

### Content excluded
- **Related Stories** (`PagePromo`/`PageList`/`Enhancement` sections) — These sidebar promotions link to separate articles and are not part of the article content. Their images and text are intentionally filtered out.
